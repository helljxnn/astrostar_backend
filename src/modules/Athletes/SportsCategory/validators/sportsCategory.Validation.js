...
export const sportsCategoryValidators = {
  ...
  create: [
    ...
    body('minAge').notEmpty().withMessage('La edad mínima es obligatoria.').isInt({ min: 4, max: 29 }).withMessage('La edad mínima debe estar entre 4 y 29 años.').toInt(),
    body('maxAge').notEmpty().withMessage('La edad máxima es obligatoria.').isInt({ min: 5, max: 30 }).withMessage('La edad máxima debe estar entre 5 y 30 años.')
      .custom((value, { req }) => {
        if (parseInt(value) <= parseInt(req.body.minAge)) throw new Error('La edad máxima debe ser mayor que la mínima.');
        return true;
      }).toInt(),
    body('status').optional().isIn(['Activo', 'Inactivo', 'Active', 'Inactive']).withMessage('El estado debe ser "Activo", "Inactivo", "Active" o "Inactive".').default('Activo'),
  ...
  update: [
    ...
    body('minAge').optional().isInt({ min: 4, max: 29 }).withMessage('La edad mínima debe estar entre 4 y 29 años.').toInt(),
    body('maxAge').optional().isInt({ min: 5, max: 30 }).withMessage('La edad máxima debe estar entre 5 y 30 años.')
      .custom((value, { req }) => {
    ...
    body('status').optional().isIn(['Activo', 'Inactivo', 'Active', 'Inactive']).withMessage('El estado debe ser "Activo", "Inactivo", "Active" o "Inactive".'),
