...
export class AppointmentController {
  ...
  /**
   * Proponer reagendamiento
   */
  proposeReschedule = async (req, res) => {
    try {
      const { id } = req.params;
      const { newDate, newStartTime, newEndTime } = req.body;

      const result = await this.appointmentService.proposeReschedule(
        parseInt(id),
        newDate,
        newStartTime,
        newEndTime
      ...
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor al proponer reagendamiento.',
        error: process.env.NODE_ENV === 'development' ? error.message : undefined
      });
    }
  };
  ...
  acceptReschedule = async (req, res) => {
    try {
      ...
      const result = await this.appointmentService.acceptReschedule(token);
      ...
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor al aceptar reagendamiento.',
        error: process.env.NODE_ENV === 'development' ? error.message : undefined
      });
    }
  };
  ...
  rejectReschedule = async (req, res) => {
    try {
      ...
      const result = await this.appointmentService.rejectReschedule(token);
      ...
      res.status(500).json({
        success: false,
        message: 'Error interno del servidor al rechazar reagendamiento.',
        error: process.env.NODE_ENV === 'development' ? error.message : undefined
      });
    }
  };
